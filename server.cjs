...
app.get('/api/users', (req, res) => {
    db.all("SELECT id, username, name, location, avatar, is_ghost FROM users", (err, rows) => {
...
app.get('/api/users/:id', (req, res) => {
    db.get("SELECT id, username, name, location, avatar, is_ghost FROM users WHERE id = ?", [req.params.id], (err, row) => {
        if (err) return res.status(500).json({ error: err.message });
        if (row && row.is_ghost) {
        ...
        [user_id, content, image, hasPwr, original_post_id || null, dropRad, hasPollVal, poll_question || null, isEmergencyVal], function(err) {
            ...
            if (isEmergencyVal) {
                db.all("SELECT id FROM users WHERE id != ?", [user_id], (err, allUsers) => {
                    (allUsers || []).forEach(u => {
                        db.run("INSERT INTO notifications (user_id, sender_id, type) VALUES (?, ?, ?)", [u.id, user_id, 'emergency']);
                    });
                });
            }
