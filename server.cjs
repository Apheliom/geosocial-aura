...
app.post('/api/register', (req, res) => {
    ...
    db.run("INSERT INTO users (username, password, name, location, avatar, last_seen) VALUES (?, ?, ?, ?, ?, ?)", [username, password, name, location, avatar, now], function(err) {
        ...
        res.json({ id: this.lastID, username, password, name, location, avatar, is_ghost: 0 });
    });
});

app.post('/api/login', (req, res) => {
    const { username, password } = req.body;
    db.get("SELECT id, username, password, name, location, avatar, is_ghost, interests, bio FROM users WHERE username = ? AND password = ?", [username, password], (err, row) => {
        if (row) {
...
app.get('/api/users', (req, res) => {
    db.all("SELECT id, username, name, location, avatar, latitude, longitude, last_seen, is_ghost, interests, bio FROM users", (err, rows) => {
...
app.get('/api/users/:id', (req, res) => {
    db.get("SELECT id, username, name, location, avatar, latitude, longitude, last_seen, is_ghost, interests, bio FROM users WHERE id = ?", [req.params.id], (err, row) => {
        if (err) return res.status(500).json({ error: err.message });
...
app.post('/api/users/:id/update', upload.single('avatar'), (req, res) => {
    const { name, username, password, bio, interests } = req.body;
    const userId = req.params.id;
    updateLastSeen(userId);
    
    if (req.file) {
        const avatarPath = '/uploads/' + req.file.filename;
        db.run("UPDATE users SET name = ?, username = ?, password = COALESCE(NULLIF(?, ''), password), bio = ?, interests = ?, avatar = ? WHERE id = ?", 
            [name, username, password, bio || '', interests || '', avatarPath, userId], function(err) {
            if (err) return res.status(500).json({ error: err.message });
            res.json({ success: true, avatar: avatarPath, name, username, password, bio, interests });
        });
    } else {
        db.run("UPDATE users SET name = ?, username = ?, password = COALESCE(NULLIF(?, ''), password), bio = ?, interests = ? WHERE id = ?", 
            [name, username, password, bio || '', interests || '', userId], function(err) {
            if (err) return res.status(500).json({ error: err.message });
            res.json({ success: true, name, username, password, bio, interests });
        });
    }
});
...
app.get('/api/posts', (req, res) => {
    ...
    const query = `
        SELECT p.*, u.username, u.name, u.avatar, 
               CASE WHEN u.location IS NULL OR u.location = 'Anónimo' THEN 'Iribarren' ELSE u.location END as location, 
               u.is_ghost,
...
app.get('/api/stats/map', (req, res) => {
    const query = `
        SELECT CASE WHEN u.location IS NULL OR u.location = 'Anónimo' THEN 'Iribarren' ELSE u.location END as location, p.has_power, COUNT(DISTINCT u.id) as users_count
        FROM posts p
        JOIN users u ON p.user_id = u.id
        WHERE p.id IN (SELECT MAX(id) FROM posts GROUP BY user_id)
        GROUP BY location, p.has_power
    `;
