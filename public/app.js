...
function requestUserGeolocation() {
    ...
    async function processCoords(lat, lon, accuracy) {
        ...
        // Auto-detect municipality when GPS accuracy is good (<1000m) OR if the user's location is currently Anónimo
        if ((accuracy !== null && accuracy < 1000) || user.location === "Anónimo") {
            const closest = findClosestMunicipality(lat, lon);
            const override = localStorage.getItem('locationOverride') === 'true';
            if (closest && closest !== user.location && (!override || user.location === "Anónimo")) {
                console.log(`Auto-matching municipality by GPS (accuracy: ${accuracy}m): ${closest} (previously: ${user.location})`);
                try {
                    const res = await fetch(`${API_URL}/users/${user.id}/update_location`, {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ location: closest })
                    });
                    if (res.ok) {
                        user.location = closest;
            ...
            console.log(`Skipping municipality auto-detect: accuracy too low (${accuracy}m). Keeping: ${user.location}`);
        }
    ...
    async function runIPFallback() {
        ...
        try {
            ...
            const loc = (user.location && user.location !== "Anónimo") ? user.location : "Iribarren";
            const meta = MUNICIPALITY_METADATA[loc];
            const coords = parseCoords(meta?.coords);
            if (coords) {
                const offsetLat = ((user.id * 17) % 100 - 50) * 0.00001;
                const offsetLon = ((user.id * 23) % 100 - 50) * 0.00001;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (settingsModal && user) {
        ...
        // Setup interests pills toggle if they exist
        if (pills && pills.length > 0) {
            pills.forEach(pill => {
                pill.addEventListener('click', () => {
                    ...
                    const interestsInput = document.getElementById('settings-interests-input');
                    if (interestsInput) interestsInput.value = selected.join(',');
                });
            });
        }
        ...
        triggers.forEach(t => t.addEventListener('click', () => {
            ...
            // Init interests from user object if elements exist
            const interestsInput = document.getElementById('settings-interests-input');
            if (interestsInput && pills && pills.length > 0) {
        ...
        document.getElementById('form-settings').addEventListener('submit', async (e) => {
            ...
            const usernameInput = document.getElementById('settings-username');
            const username = usernameInput ? usernameInput.value : user.username;
            const passwordInput = document.getElementById('settings-password');
            const password = passwordInput ? passwordInput.value : user.password;
            const bioInput = document.getElementById('settings-bio');
            const bio = bioInput ? bioInput.value : user.bio;
            
            const interestsInput = document.getElementById('settings-interests-input');
            const interests = interestsInput ? interestsInput.value : (user.interests || '');
            ...
            posts.forEach(p => {
                ...
                <div class="p-3 flex items-center justify-between border-b border-outline-variant/50">
                    <div class="flex items-center gap-3">
                        <div class="w-8 h-8 rounded-full overflow-hidden border ${p.has_power ? 'border-primary' : 'border-error'} relative cursor-pointer btn-view-profile" data-user-id="${p.user_id}">
                            <img alt="Author" class="w-full h-full object-cover" src="${p.avatar}"/>
                        </div>
                        <div class="flex flex-col">
                            <span class="font-status-code text-sm text-on-surface cursor-pointer btn-view-profile" data-user-id="${p.user_id}">${p.name} <span class="text-outline text-xs">@${p.username}</span></span>
    ...
    if (btnCreateMeetup && createMeetupModal) {
        btnCreateMeetup.addEventListener('click', () => {
            ...
            const detectedLocation = findClosestMunicipality(myLat, myLon) || (user && user.location) || 'Iribarren';
            document.getElementById('meetup-location-lbl').textContent = detectedLocation.toUpperCase();
        ...
        btn.addEventListener('click', async () => {
            ...
            if (zoneCoords) {
                ...
                try {
                    ...
                    const closest = findClosestMunicipality(realUserCoords.lat, realUserCoords.lon);
                    if (closest) {
                        const locRes = await fetch(`${API_URL}/users/${user.id}/update_location`, {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({ location: closest })
                        });
        ...
        async function loadNodes() {
            try {
                ...
                processedNodes.forEach(({ user: u, dist }) => {
                    ...
                    li.innerHTML = `
                        <div class="flex items-center gap-3">
                            <img src="${u.avatar}" class="w-8 h-8 rounded-full border border-primary object-cover cursor-pointer btn-view-profile-avatar" data-user-id="${u.id}">
                            <div>
                ...
                document.querySelectorAll('.btn-view-profile-avatar').forEach(img => {
    ...
    async function openUserProfile(userId) {
        ...
        document.getElementById('profile-name').textContent = 'Cargando...';
        document.getElementById('profile-username').textContent = '';
        document.getElementById('profile-online-badge').className = 'hidden';
        document.getElementById('profile-sector').innerHTML = `<span class="material-symbols-outlined text-[11px]">location_on</span> SECTOR: CARGANDO...`;
        document.getElementById('profile-bio').textContent = '';
        
        // Hide badges initially
        document.getElementById('profile-badge-explorer').classList.add('hidden');
        document.getElementById('profile-badge-hacker').classList.add('hidden');
        document.getElementById('profile-badge-operator').classList.add('hidden');
        
        // Clear grid
        const postsGrid = document.getElementById('profile-posts-grid');
        postsGrid.innerHTML = '<div class="col-span-full text-center text-xs text-outline py-8">Cargando publicaciones...</div>';
        
        // Add Message Button if not the current logged in user
        let messageBtn = document.getElementById('profile-msg-btn');
        if (messageBtn) messageBtn.remove();
        
        if (user && parseInt(userId) !== parseInt(user.id)) {
            // Find parent detail container to add the button
            const detailContainer = document.querySelector('#profile-modal #profile-name').parentElement.parentElement;
            messageBtn = document.createElement('button');
            messageBtn.id = 'profile-msg-btn';
            messageBtn.className = 'mt-3 px-4 py-1.5 bg-primary/20 text-primary border border-primary/40 text-[9px] font-label-caps rounded hover:bg-primary/30 transition-all w-fit';
            messageBtn.textContent = 'MENSAJE';
            messageBtn.addEventListener('click', () => {
                window.location.href = `/nodes?chat_user=${userId}`;
            });
            detailContainer.appendChild(messageBtn);
        }
        ...
        try {
            ...
            if (userRes.ok) {
                ...
                document.getElementById('profile-avatar').src = uData.avatar || '';
                document.getElementById('profile-name').textContent = uData.name || '';
                document.getElementById('profile-username').textContent = `@${uData.username}`;
                
                const locStr = (uData.location && uData.location !== 'Anónimo') ? uData.location.toUpperCase() : 'IRIBARREN';
                document.getElementById('profile-sector').innerHTML = `<span class="material-symbols-outlined text-[11px]">location_on</span> SECTOR: ${locStr}`;
                
                document.getElementById('profile-bio').textContent = uData.bio || 'Sin biografía disponible.';
                
                const badge = document.getElementById('profile-online-badge');
                if (uData.is_online) {
                    badge.textContent = 'CONECTADO';
                    badge.className = 'font-status-code text-[8px] px-1.5 py-0.5 border border-primary/30 bg-primary/5 rounded w-fit mx-auto sm:mx-0 text-primary';
                } else {
                    badge.textContent = 'DESCONECTADO';
                    badge.className = 'font-status-code text-[8px] px-1.5 py-0.5 border border-outline/35 bg-surface-dim rounded w-fit mx-auto sm:mx-0 text-outline';
                }
            }

            // 2. Fetch User RPG Status (badges)
            const rpgRes = await fetch(`${API_URL}/rpg/status/${userId}`);
            if (rpgRes.ok) {
                const rpgData = await rpgRes.json();
                if (rpgData.medals) {
                    if (rpgData.medals.includes('EXPLORADOR_NEON')) {
                        document.getElementById('profile-badge-explorer').classList.remove('hidden');
                    }
                    if (rpgData.medals.includes('HACKER_TEMPORAL')) {
                        document.getElementById('profile-badge-hacker').classList.remove('hidden');
                    }
                    if (rpgData.medals.includes('OPERADOR_NET')) {
                        document.getElementById('profile-badge-operator').classList.remove('hidden');
                    }
                }
            }

            // 3. Fetch Posts & Filter by User
            let myLat = null;
            let myLon = null;
            if (realUserCoords) {
                myLat = realUserCoords.lat;
            ...
            if (myLat !== null && myLon !== null) {
                postsUrl += `&user_lat=${myLat}&user_lon=${myLon}`;
            }

            const postsRes = await fetch(postsUrl);
            if (postsRes.ok) {
                const posts = await postsRes.json();
                const userPosts = posts.filter(p => parseInt(p.user_id) === parseInt(userId));
                
                postsGrid.innerHTML = '';
                if (userPosts.length === 0) {
                    postsGrid.innerHTML = '<div class="col-span-full text-center text-xs text-outline py-8">No hay transmisiones disponibles de este nodo.</div>';
                } else {
                    userPosts.forEach(p => {
                        const card = document.createElement('div');
                        card.className = 'bg-surface p-3 rounded border border-outline-variant/40 flex flex-col gap-2 hover:border-primary/30 transition-all text-xs';
                        
                        let mediaContent = '';
                        if (p.image) {
                            mediaContent = `<img src="${p.image}" class="w-full h-24 object-cover rounded border border-outline-variant/30 mb-1">`;
                        }
                        
                        card.innerHTML = `
                            ${mediaContent}
                            <p class="font-body-md text-on-surface-variant flex-1 line-clamp-3">${p.content}</p>
                            <div class="flex justify-between items-center text-[9px] text-outline mt-2 pt-1 border-t border-outline-variant/20 font-status-code">
            ...
            postsGrid.innerHTML = '<div class="col-span-full text-center text-xs text-error py-8">Fallo al sintonizar con el nodo de telemetría.</div>';
        }
    }

    // Set globally for triggers in HTML
    window.openUserProfile = openUserProfile;

    // Wire profile close trigger
    ...
    // Wire navbar username click to open own profile modal
    document.querySelectorAll('.current-user-name').forEach(el => {
        el.classList.add('cursor-pointer', 'hover:text-primary-fixed-dim');
