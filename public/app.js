...
function requestUserGeolocation() {
    ...
    async function processCoords(lat, lon) {
        ...
        console.log("Telemetry Coordinates locked:", realUserCoords);
        ...
        // Determine closest Lara state municipality
        const closest = findClosestMunicipality(lat, lon);
        if (closest && closest !== currentUser.location) {
            console.log(`Auto-matching municipality by coordinates: ${closest} (previously: ${currentUser.location})`);
            try {
    ...
    async function uploadCurrentPosition() {
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(
                ...
                async (error) => {
                    console.warn("GPS Geolocation failed. Initiating IP fallback lookup...", error);
                    try {
                        const ipRes = await fetch('https://ip-api.com/json/');
                        const ipData = await ipRes.json();
                        if (ipData && ipData.lat !== undefined && ipData.lon !== undefined) {
                            processCoords(ipData.lat, ipData.lon);
                        } else {
                            throw new Error("Invalid IP geo payload");
                        }
                    } catch (ipErr) {
                        console.warn("IP geolocation fallback failed. Utilizing municipality default.", ipErr);
                        // Use municipality coordinates as fallback with a unique offset
                        const meta = MUNICIPALITY_METADATA[currentUser.location];
                        const coords = parseCoords(meta?.coords);
                        if (coords) {
                            const offsetLat = ((currentUser.id * 17) % 100 - 50) * 0.0005;
                            const offsetLon = ((currentUser.id * 23) % 100 - 50) * 0.0005;
                            processCoords(coords.lat + offsetLat, coords.lon + offsetLon);
                        }
                    }
                },
    ...
    // Run periodically every 5 seconds for real-time tracking
    setInterval(uploadCurrentPosition, 5000);
}
    ...
    async function updateRadar() {
        ...
        try {
            ...
            function getUserCoordinates(uId, uLocation, uLatitude, uLongitude) {
                let lat = uLatitude;
                let lon = uLongitude;
                if (uLocation && uLocation !== 'Anónimo') {
                    const meta = MUNICIPALITY_METADATA[uLocation];
                    const coords = parseCoords(meta?.coords);
                    if (coords) {
